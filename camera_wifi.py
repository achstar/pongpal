...
class State(Enum):
    ...
    LEFT_BOUNCE = 2
    RIGHT_BOUNCE = 3
pico_ips = ["172.20.10.2", "172.20.10.3"]
sockets = []
pico_id = 0
HOST = 5011
while(len(sockets) < 2):
    try:
        # Create a socket and connect to the Pico
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        print(pico_ips[pico_id])
        client_socket.connect((pico_ips[pico_id], HOST))
        sockets.append({"socket": client_socket})
        print(f"Connected to Pico W at {pico_ips[pico_id], HOST}")
        pico_id += 1
    except Exception as e:
        print(f"Failed to connect to Pico")
...
while True:
    ...
    if (left_point):
        ...
        pin_is_high = True
        left_score += 1
        sockets[0]["socket"].send("1".encode())
        sockets[1]["socket"].send("0".encode())
        ...
        pin_is_high = True
        right_score += 1
        sockets[0]["socket"].send("0".encode())
        sockets[1]["socket"].send("1".encode())
